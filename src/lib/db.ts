  ...
  try {
    ...
    if (effectivePlanName !== "") {
      ...
      if (result.rowCount === 0) {
        ...
        throw new Error(`Meal plan "${effectivePlanName}" not found.`);
      }
    }
    ...
    await sql.query('ROLLBACK');
    console.error("Database error: Failed to set active plan:", error);
    throw new Error("Failed to set active meal plan in the database.");
  }
}
...
export async function saveShoppingListToDb(planName: string, shoppingListText: string): Promise<void> {
  ...
  if (!shoppingListText || shoppingListText.trim() === "") {
    throw new Error("Shopping list text cannot be empty.");
  }
  try {
    await sql`
    ...
    console.error("Database error: Failed to save shopping list:", error);
    throw new Error("Failed to save shopping list to the database.");
  }
}
...
export async function saveMealPlanAnalysisToDb(planName: string, analysisText: string): Promise<void> {
  ...
  if (!analysisText || analysisText.trim() === "") {
    throw new Error("Analysis text cannot be empty.");
  }
  try {
    const result = await sql`
      UPDATE meal_plans
    ...
    console.error("Database error: Failed to save meal plan analysis:", error);
