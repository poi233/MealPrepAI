...
export default function AddRecipeDialog({
  ...
  const handleGenerateDetails = async () => {
    ...
    try {
      const result = await generateRecipeDetailsAction(recipeName, mealTypeTitle);
      if ("error" in result) {
        toast({
          title: "AI详情生成错误",
        ...
        form.setValue("ingredients", result.recipe.ingredients.join("\n"));
        form.setValue("instructions", result.recipe.instructions);
        toast({
          title: "食谱详情已生成",
      ...
      toast({
        title: "错误",
        description: error instanceof Error ? error.message : "生成食谱详情失败。",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSuggestRecipe = async () => {
    ...
    form.reset(); 
    try {
      const result = await suggestRecipeAction(mealTypeTitle, planDescription);
      if ("error" in result) {
        toast({
          title: "AI推荐错误",
          description: result.error,
        ...
        form.setValue("recipeName", firstSuggestion.name);
        form.setValue("ingredients", firstSuggestion.ingredients.join("\n"));
        form.setValue("instructions", firstSuggestion.instructions);
        toast({
          title: "食谱已推荐",
          description: `AI已推荐 "${firstSuggestion.name}" 并填充了详情。`,
        });
      }
    } catch (error) {
      toast({
        title: "错误",
        description: error instanceof Error ? error.message : "推荐食谱失败。",
        variant: "destructive",
      });
    } finally {
